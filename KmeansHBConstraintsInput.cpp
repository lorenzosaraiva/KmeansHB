...
int main(int argc, char** argv){
  ...
  ifstream constraintsFile(solutionName);
  //constraintsFile.open("solutions/fisher/solution10.txt");
  ...
  int mustLinks;
  int cannotLinks;
  constraintsFile >> clusters;
  constraintsFile >> f_best;
  constraintsFile >> mustLinks
  constraintsFile >> cannotLinks;

  Constraints *constraints = new Constraints(n);

  // Read the assignments
  for(int i = 0; i < mustLinks; i++){
      int a, b;
      double w;
      constraintsFile >> a;
      constraintsFile >> b;
  ...
  while(counter < cruns){
    ...
    //cout << "Best solution for " << clusters << " clusters costs " << best << " with gap " << gap << " and time " << elapsed_secs << " - " << runs << " runs" << endl;
