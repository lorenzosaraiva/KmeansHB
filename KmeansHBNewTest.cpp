...
static bool sortGreaterThan(double u, double v)
{
   return u > v;
}
...
int main(int argc, char** argv){
  ...
  while(counter < 1){
    ...
    for(int i = 0; i < runs; i++){
      ...
      double gap = 100 * (solutionCost - f_best) / f_best;
      totalGap += gap;
      gapVector.push_back(gap);
    ...
    double bestGap = 100 * (best - f_best) / f_best;
    double worstGap = 100 * (worst - f_best) / f_best;
    double avgGap = totalGap/runs;
    std::sort(gapVector.begin(), gapVector.end(), sortGreaterThan);
    double medianGap = gapVector[runs/2];
    ...
    fileName = fileName + "_" + to_string(runs) + "_" + to_string(cAmountMultiplier) + "_" + to_string(cWeightMultiplier) + "_" + to_string(clusters) + ".txt";

    std::ofstream myfile (fileName);

    myfile << fixed << bestGap << " "; 
    myfile << fixed << worstGap << " "; 
    myfile << fixed << avgGap << " ";
    myfile << fixed << medianGap << " ";
    myfile << clusters << " "; 
